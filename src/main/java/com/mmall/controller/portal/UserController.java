...
//            CookieUtil.readLoginToken(httpServletRequest);
            ...
            RedisPoolUtil.setEx(session.getId(), JsonUtil.ojb2String(response.getData()), Const.RedisCacheExtime.REDIS_SESSION_EXTIME);
        }
        return response;
    }
    ...
    public ServerResponse<User> update_information(HttpServletRequest request, User user) {
        ...
        if (org.springframework.util.StringUtils.isEmpty(loginToken)) {
            ...
            if (response.isSuccess()) {
                ...
                RedisPoolUtil.setEx(loginToken, JsonUtil.ojb2String(response.getData()), Const.RedisCacheExtime.REDIS_SESSION_EXTIME);
            }
            return response;
        }
    }
